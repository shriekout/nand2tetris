# Nand2Tetris

This repository contains my solutions and implementations for the **Nand2Tetris** project.

- **Official website:** <https://www.nand2tetris.org/>
- **Book (Korean):** *밑바닥부터 만드는 컴퓨팅 시스템 2판 (불 논리부터 컴퓨터 아키텍처, 운영체제까지)*  
  노암 니산(Noam Nisan), 시몬 쇼켄(Shimon Schocken) 지음 / 김진홍 옮김 / 인사이트
- **Original book:** *The Elements of Computing Systems: Building a Modern Computer from First Principles* by Noam Nisan and Shimon Schocken

## Progress

### Part I – Hardware

- [x] Chapter 1: Boolean Logic
- [x] Chapter 2: Boolean Arithmetic
- [x] Chapter 3: Memory
- [x] Chapter 4: Machine Language
- [x] Chapter 5: Computer Architecture
- [x] Chapter 6: Assembler

### Part II – Software

- [x] Chapter 7: Virtual Machine I – Processing
- [x] Chapter 8: Virtual Machine II – Control
- [x] Chapter 9: High-Level Language
- [x] Chapter 10: Compiler I – Syntax Analysis
- [x] Chapter 11: Compiler II – Code Generation
- [ ] Chapter 12: Operating System
- [ ] Chapter 13: More Fun to Go

## Learning Environment

* WSL / Debian
* GCC / Make
* VS code
* Git
* Java (if using official tools)

## Acknowledgements

This project documents my Nand2Tetris learning journey.

ChatGPT was used as a learning assistant to clarify concepts in computer architecture, C programming, and Git. All implementations and design decisions were independently made.
